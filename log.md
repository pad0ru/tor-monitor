...
### Security hardening pass (v0.3.1)

Threat-modelled the app and built out the missing safeguards. Scope: defense in depth (treat the relay and its output as potentially hostile) and folding in the two security-relevant items from the earlier code review.

- **Electron lockdown (`main.js`).** Added a global `web-contents-created` handler that denies pop-ups (`setWindowOpenHandler`), off-`file://` navigation (`will-navigate`), `<webview>` embedding, and all device-permission requests. Set `sandbox: true` explicitly on both the main and tool windows (was the Electron default; now durable). Limits the blast radius if injected content ever tried to escape a page.
- **Prototype-pollution-safe parsers.** `parseKeyValues`, `parsePairsLine`, `parseMeminfo` (main.js) and `repliesToMap` (relay-agent) now build `Object.create(null)` accumulators and drop `__proto__`/`constructor`/`prototype` keys, so a hostile relay emitting such keys in `os-release`/`lscpu`/`lsblk`/ControlPort replies can't pollute `Object.prototype`.
- **Bounded exec timeout (DoS).** `sshExec`'s timeout timer now starts *before* `sshClient.exec()` instead of inside its callback, so a stall during channel-open/exec-request negotiation is bounded too (previously it could hang a polling window indefinitely). On timeout it removes the stream's listeners and closes the channel so orphaned channels don't accumulate across the poll loop. `EXEC_TIMEOUT_MS` is now overridable via env (for tests).
- **Kill-the-right-process (safety).** The task manager tracks the selected process by pid **and** name. If a pid is recycled for a differently-named process between refreshes, or the process exits, the selection is dropped and the End/Force-kill buttons disable — closing a window where Force kill could have signalled an unrelated process. Selection clearing now works under an active filter too (checks the full process list, not just visible rows).
- **Confirmed already-covered, no change:** SSH host-key TOFU pinning, in-memory-only credentials with no logging, localhost-bound agent with constant-time token compare, strict CSP + `textContent` rendering, `Number.isInteger` pid validation on kill, 4 MB output cap, HTTPS Onionoo.
- **Tests:** integration adds 4 checks (poisoned specs parse correctly with no `Object.prototype` pollution; a stalled exec times out within bound and the connection recovers); e2e adds 3 (a real process ended via the UI removes its row and clears the selection; `window.open` denied with no popup; external navigation blocked, stays on `file://`). Both suites pass, and the hardened app was re-verified live against the relay. A new **Security model** section documents the posture in `electron-app/README.md`.

### Security-hardening verification pass

Built out dedicated, independent verification for the three v0.3.1 safeguards: 72 checks (30 headless + 42 real-Electron) covering good flow, alternate flow, and adversarial/bad flow for each, on top of what the hardening commit's own tests already covered.

- **`test/security-hardening.test.js` (30, headless).** A scriptable mock sshd injects exact attack payloads per check: `__proto__`/`constructor`/`prototype` poisoning of every parsed specs field individually and combined, the same attempt against the relay-agent's own `repliesToMap` (verified in isolation via a `vm` sandbox against the actual source, since requiring the agent module directly triggers its side effects), a stalled exec channel (must time out, not hang — confirmed the timer window is tight: fires within the configured bound, not instantly, not late), a relay that refuses the channel outright, an output flood (stays capped, resolves promptly), a dotted `__proto__.__proto__` key (lands as an inert property — the null-prototype backstop covers it even though the explicit skip-list doesn't match it exactly), and the `sysinfo:kill` IPC boundary directly (PID 1, non-integer/injection-shaped and negative/zero pids, arbitrary signal strings coerced to TERM, concurrent End+Force-kill on the same pid).
- **`test/security-e2e.js` (42, real Electron).** Popup denial across five URL schemes plus a positive control proving same-app `file://` navigation still works; external navigation blocked via five distinct vectors (`location.replace`/`assign`, an external `<a target=_blank>`, a `<form>` submit, an injected `<meta refresh>`, `window.top.location`); camera/geolocation/notification permission requests denied; `<webview>` inert; renderer still has no Node globals under the newly-explicit `sandbox: true`; all contextBridge APIs still function under sandbox. Task-manager kill safety exercised two ways: real child processes ended through the actual UI/IPC/SSH path (including a confirm-then-cancel case and a since-exited-process race that must report "no longer exists" cleanly), and a second, mock-driven task manager window — the real unmodified `taskmanager.html`/`.js`, fed scripted snapshots over a throwaway IPC channel — to deterministically prove the core fix (a pid recycled by a differently-named process clears the selection instead of re-targeting a kill at it) plus a positive control (same pid + same name persists) and a documented limitation (same pid + identical name is genuinely indistinguishable).
- **Bugs caught in the tests themselves, not the app:** an initial pid-reuse assertion mis-modeled how `Object.create(null)` objects report `.__proto__`/`.constructor` (they return `undefined`, not `Object.prototype`, since there's no inherited accessor) — fixed to check `hasOwnProperty` and `Object.getPrototypeOf(result) === null` instead. A rapid-reselection test clicked stale, already-detached `<tr>` references after `renderTable()` replaced the `<tbody>` — fixed to re-query the live DOM before each click. A stale-selection race test read the status bar before actually clicking Confirm, and separately didn't notice the task-manager window's own background 2s auto-poll (running since it opened, independent of the test's explicit `refresh()` calls) was racing in and overwriting the error message it was checking for — fixed by pausing that window's interval for the section.
- All suites (these two plus the existing `integration.test.js` and `e2e-electron.js`) pass together with no regressions, run twice to confirm no flakiness.
- **Results:** `security-hardening.test.js` — 30/30 passed (two runs). `security-e2e.js` — 42/42 passed (two runs); first run caught the 3 test-design bugs above (2 failures the first pass, fixed, then 0/42 failures on rerun). `integration.test.js` and `e2e-electron.js` — unchanged, still fully passing. **72/72 total.**
